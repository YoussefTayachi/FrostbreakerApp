...
export const TOOL_DESCRIPTIONS: Record<ToolName, string> = {
  ...
  set_lead_icebreaker:
    "Sets the icebreaker of exactly one lead. Requires workspace_id, business_id and icebreaker (the new text to store). Writes one lead per call; for a whole list, use set_lead_icebreakers, which takes up to 50 named leads at once. Overwrites whatever was there before, including text a person wrote by hand in the app. Needs a token with the read_write scope, and every call is recorded in a permanent log with the old and new value. The response carries the log_id of that entry: pass it to undo_writes in log_ids to revert exactly this write, instead of guessing a time window.",
