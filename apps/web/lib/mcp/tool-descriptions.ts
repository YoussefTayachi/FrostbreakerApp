...
export const TOOL_DESCRIPTIONS: Record<ToolName, string> = {
   ...
   */
  set_lead_website_finding:
    "Sets the website finding of exactly one lead: the one verified flaw on that company's website, written as a single sentence with its consequence. This is NOT the icebreaker and does not go in the same field: the icebreaker is the opening hook ('that you have tripled your locations since the move'), the finding is the measured defect ('your site takes eleven seconds to load on a phone'). A sequence can use both, as {{icebreaker}} and {{websiteFinding}}, which is why they are kept apart. Requires workspace_id, business_id and website_finding. State the flaw as a fact without hedging, name no second flaw and no invented numbers, do not say how you know it, and do not pitch a service: the sentence names the problem, not the solution. Call get_writing_rules first, but note the finding has its OWN word limit of 20, lower than the icebreaker's, while the banned words and the output language are the workspace's usual ones; a sentence that breaks any of them is still written and then flagged for review in Frostbreaker, where a person has to deal with it. An empty string clears the field, and a lead with an empty finding is held back from any campaign whose sequence uses {{websiteFinding}}. Writes one lead per call; there is no bulk form, because each finding needs its own look at that website. Needs a token with the read_write scope, and every call is recorded in a permanent log with the old and new value; the response carries that entry's log_id for undo_writes.",
