...
import { groupByWeek, isoWeekNumber, type Sender } from "@/lib/wiederkontakt";
...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => ({}))) as { week?: string; sender?: string };
  ...
  const waiting = await loadWaiting(supabase, ws.workspace.id);
  const bucket = groupByWeek(waiting, new Date()).find((b) => b.week === week && b.sender === sender);
  if (!bucket) return NextResponse.json({ error: "Diese Woche hat keine Wartenden mehr" }, { status: 404 });

  try {
    const result = await createBucket(supabase, ws.workspace.id, week, sender, bucket.contacts.map((c) => c.id));
