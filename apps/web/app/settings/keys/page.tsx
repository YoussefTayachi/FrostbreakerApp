        ...
        {PROVIDER_IDS.map((p) => (
          <div key={p} className={cardCls}>
            <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
              <h2 className="text-base font-semibold text-ink">{providerLabels[p]}</h2>
              {saved.includes(p) && (
                <span className="flex min-w-0 items-center gap-2">
                  {keyHints[p] && (
                    <code className="truncate rounded-md bg-chip px-2 py-1 font-mono text-2xs text-faint">
                      {keyHints[p]}
                    </code>
                  )}
                  <span className="flex shrink-0 items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-600 dark:text-emerald-300">
            ...
            <p className="mb-4 text-sm leading-relaxed text-faint">{t.settings.providerHints[p]}</p>
            {/* Unter sm steht das Feld allein in einer Zeile und die Knoepfe
                darunter: nebeneinander waren es bis zu drei Knoepfe neben
                einem Eingabefeld, was auf 390 Pixeln nicht aufgeht. */}
            <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center">
              ...
              <div className="flex flex-wrap gap-2.5">
                <button onClick={() => save(p)} className={primaryBtnCls}>
                  {t.settings.save}
                </button>
                {TESTABLE.includes(p) && saved.includes(p) && (
                  <button
                    ...
                    className={
                      "rounded-lg border px-4 py-2.5 text-sm font-medium transition-[background-color,border-color,transform] duration-150 active:scale-[0.98] disabled:opacity-50 " +
                      (keyTest[p] === "ok"
                ...
                {saved.includes(p) && (
                  <button onClick={() => remove(p)} disabled={removing === p} className={dangerBtnCls}>
      ...
      >
        <span className="flex min-w-0 items-center gap-3">
          ...
          <span className="min-w-0">
            ...
            <span className="mt-0.5 block text-sm leading-relaxed text-faint">API-Key, Mailboxen und Kampagnen jetzt im eigenen Bereich verwalten</span>
          </span>
        </span>
