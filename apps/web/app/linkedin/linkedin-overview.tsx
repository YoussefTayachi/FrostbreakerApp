...
export default function LinkedInOverview({
  ...
  return (
    <div className="space-y-5">
      {truncated && (
        <p className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-3.5 py-2.5 text-sm text-amber-700 dark:text-amber-500">
      ...
      {totals.followUpsDue > 0 && (
        <p className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-3.5 py-2.5 text-sm font-medium text-amber-700 dark:text-amber-500">
      ...
      <div>
        <h2 className="mb-3 text-base font-semibold text-ink">{L.listsHeading}</h2>
        {lists.length === 0 ? (
...
function Stat({
  ...
  return (
    <div className="rounded-xl border border-edge/70 bg-panel px-4 py-3.5 shadow-sm">
...
function ListCard({ list }: { list: LeadListSummary }) {
  ...
  return (
    <Link
      ...
      className="group block rounded-xl border border-edge/70 bg-panel p-4 shadow-sm transition-[border-color,box-shadow] duration-150 hover:border-sky-500/50 hover:shadow-md"
    >
      <div className="mb-2 flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-base font-semibold text-ink transition-colors group-hover:text-sky-600 dark:group-hover:text-sky-400">
            {list.name}
          </p>
          <p className="mt-0.5 truncate text-xs text-faint">{list.location ?? "—"}</p>
        </div>
        <SourceBadge source={list.source} />
      </div>

      {list.note && <p className="mb-2 line-clamp-2 text-xs text-mute">{list.note}</p>}
      ...
      <p className="mt-1.5 text-xs tabular-nums text-mute">{L.cardProgress(list.contacted, list.total)}</p>
    </Link>
  );
}
